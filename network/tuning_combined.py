...
from training_old import load_model, load_trainer, planning, load_dataset, states_to_graphs2

domain_to_train_set = {
    "gripper": Path('data_old/supervised/optimal/train/gripper-atomic/gripper-atomic/'),
    "blocks-clear": Path('data_old/supervised/optimal/train/blocks-clear/blocks-clear/'),
    "blocks-on": Path('data_old/supervised/optimal/train/blocks-on/blocks-on/'),
    "visitall": Path('data_old/supervised/optimal/train/visitall-atomic/visitall-atomic/'),
    "parking-behind": Path('data_old/supervised/optimal/train/parking-behind/parking-behind/'),
    "parking-curb": Path('data_old/supervised/optimal/train/parking-curb/parking-curb/'),
    "satellite": Path('data_old/supervised/optimal/train/satellite/satellite/')
}
domain_to_validation_set = {
    "gripper": Path('data_old/supervised/optimal/validation/gripper-atomic/gripper-atomic/'),
    "blocks-clear": Path('data_old/supervised/optimal/validation/blocks-clear/blocks-clear/'),
    "blocks-on": Path('data_old/supervised/optimal/validation/blocks-on/blocks-on/'),
    "visitall": Path('data_old/supervised/optimal/validation/visitall-atomic/visitall-atomic/'),
    "parking-behind": Path('data_old/supervised/optimal/validation/parking-behind/parking-behind/'),
    "parking-curb": Path('data_old/supervised/optimal/validation/parking-curb/parking-curb/'),
    "satellite": Path('data_old/supervised/optimal/validation/satellite/satellite/')
}
...
def _parse_arguments():
    ...
    default_max_samples_per_file = 100  # TODO: INCREASE THIS?
    ...
    parser.add_argument('--domains', required=True, type=str, nargs='+', help='domain names')
...
def _main(args):
    ...
    for domain in args.domains:
        results[domain+"_instances"] = []
        results[domain+"_coverage"] = []
    ...
    for config in configs:
        ...
        for round in range(args.rounds):
            ...
            all_predicates = []
            all_decoded_predicates = []
            for domain in args.domains:
                train_dataset, predicates, decoded_predicates = load_dataset(domain_to_train_set[domain], args.max_samples_per_file)
                print(predicates)
                ...
                validation_dataset, _, _ = load_dataset(domain_to_validation_set[domain], args.max_samples_per_file)
                ...
                all_validation_samples.extend(validation_samples)
                all_predicates.extend(predicates)
                all_decoded_predicates.extend(decoded_predicates)
            ...
            for predicate, arity in all_decoded_predicates:
                decoded_predicate_dict[predicate] = arity
                decoded_predicate_ids[predicate] = i
                i += 1

            print("Predicate dict: ", predicate_dict)
            print("Predicate ids: ", predicate_ids)
            print("Max arity: ", max_arity)
            print("Decoded Predicate dict: ", decoded_predicate_dict)
            print("Decoded Predicate ids: ", decoded_predicate_ids)

            train_graphs = states_to_graphs2(all_train_samples, predicate_dict, predicate_ids, max_arity)
            validation_graphs = states_to_graphs2(all_validation_samples, predicate_dict, predicate_ids, max_arity)
        ...
        for policy_type, policy, directory in policies_and_directories:
            domain_results = {}
            for domain in args.domains:
                print(colored(f"Running policy {policy_type} on domain {domain}", 'red', attrs=['bold']))

                # load files for planning
                domain_file = Path('data_old/pddl/' + domain + '/test/domain.pddl')
                problem_files = glob.glob(str('data_old/pddl/' + domain + '/test/' + '*.pddl'))
                ...
                print(coverages)
                results[domain+"_instances"].append(len(problem_files)-1)
                results[domain+"_coverage"].append(best_coverage)

            # save results of the best run
            results["policy_path"].append(policy)
            results["val_loss"].append(best_trained_val_loss)
            for arg in vars(args):
                results[arg].append(getattr(args, arg))
            #results.update(vars(args))
            #results["num_layers_range"].append("".join([x + "," for x in map(str, args.num_layers_range)]))
            #results["hidden_size_range"].append("".join([x + "," for x in map(str, args.hidden_size_range)]))
            #results["dropout_range"].append("".join([x + "," for x in map(str, args.dropout_range)]))
            #results["heads_range"].append("".join([x + "," for x in map(str, args.heads_range)]))
