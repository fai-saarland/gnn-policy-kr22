...
def _parse_arguments(exec_path : Path):
    ...
    parser.add_argument('--model', required=False, type=Path, help='model file')
...
def _main(args, model_path, domain_file, problem_file, logger):
    ...
    try:
        ...
        model = Model.load_from_checkpoint(checkpoint_path=str(model_path), strict=False, map_location=torch.device('cpu')).to(device)
...
if __name__ == "__main__":
    ...
    for domain in args.domains:
        # model_path = glob.glob(str('../lightning_logs/' + domain + "/version_0/checkpoints/" + '*.ckpt'))[0]
        if args. model:
            model_path = args.model
        else:
            model_path = glob.glob(str('../lightning_logs/' + domain + "/version_0/checkpoints/" + '*.ckpt'))[0]
        #model_path = Path("/Users/nicola_mueller/Desktop/general_policies/lightning_logs/blocks_retrain/version_0/checkpoints/epoch=21-step=2750-validation_loss=0.00015402992721647024.ckpt")
        domain_file = Path('../data/pddl/' + domain + '/test/domain.pddl')
        problem_files = glob.glob(str('../data/pddl/' + domain + '/test/' + '*.pddl'))
        #domain_file = Path('../data/pddl/' + domain + '/validation/domain.pddl')
        #problem_files = glob.glob(str('../data/pddl/' + domain + '/validation/' + '*.pddl'))
        # domain_file = Path('../data/pddl/' + domain + '/train/domain.pddl')
        # problem_files = glob.glob(str('../data/pddl/' + domain + '/train/' + '*.pddl'))
        print(model_path)
        print(domain_file)
        print("\n")
        print(problem_files)
        ...
        for problem_file in problem_files:
            ...
            if args.retrain:
                logfile = '../plans_retrain/' + domain + '/' + logfile_name
            else:
                logfile = '../plans/' + domain + '/' + logfile_name
            log_level = logging.INFO if args.debug_level == 0 else logging.DEBUG

            logger = plan._get_logger(exec_name + problem_name, logfile, log_level)
