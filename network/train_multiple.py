...
def _parse_arguments():
    ...
    default_patience = 25  # TODO: REDUCED THIS FROM 50
    default_gradient_clip = 0.1
    default_profiler = None
    default_validation_frequency = 1
    default_save_top_k = 1

    parser.add_argument('--domains', required=True, nargs='+', help='domain names')
    ...
    parser.add_argument('--loss', nargs='?',
                        choices=['supervised_optimal', 'selfsupervised_optimal', 'selfsupervised_suboptimal',
                                 ...
                                 'online_optimal'], default='selfsupervised_suboptimal')
    ...
    parser.add_argument('--aggregation', default=default_aggregation, nargs='?',
                        choices=['add', 'max', 'addmax', 'attention'],
    ...
    parser.add_argument('--logname', default=None, type=str,
                        help='if provided, versions are stored in folder with this name inside logdir')
    parser.add_argument('--save_top_k', default=default_save_top_k, type=int,
                        help=f'number of top-k models to save (default={default_save_top_k})')

    args = parser.parse_args()
    return args

def _get_paths(args):
    train_path = "data/states/train/" + args.domain + "/" + args.domain + "/"
    val_path = "data/states/validation/" + args.domain + "/" + args.domain + "/"

def _load_datasets(args, train_path, val_path):
    ...
    (train_dataset, predicates) = load_dataset(train_path, args.max_samples_per_file, args.max_samples, args.verify_datasets)
    (validation_dataset, _) = load_dataset(val_path, args.max_samples_per_file, args.max_samples, args.verify_datasets)
    loader_params = {
...
def _load_trainer(args, domain):
    ...
    trainer_params = {
        "accelerator": "cpu",  # TODO: ADDED THIS
...
def _main(args):
    train._process_args(args)
    for domain in args.domains:
        train_path = Path("data/states/train/" + domain + "/" + domain + "/")
        val_path = Path("data/states/validation/" + domain + "/" + domain + "/")
        predicates, train_loader, validation_loader = _load_datasets(args, train_path, val_path)
        model = train._load_model(args, predicates)
        trainer = _load_trainer(args, domain)
