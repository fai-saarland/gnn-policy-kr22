...
def save_results(results, policy_type, avg_val_loss, avg_val_coverage, avg_val_avg_plan_length, num_layers, hidden_size, dropout, heads):
    results["type"].append(policy_type)
    results["avg_val_loss"].append(avg_val_loss)
    results["avg_val_coverage"].append(avg_val_coverage)
    results["avg_val_avg_plan_length"].append(avg_val_avg_plan_length)
...
def _main(args):
    ...
    for config in configs:
        ...
        for _ in range(args.seeds):
            # TODO: SAMPLE NEW DATASETS FOR EVERY SEED TO FIND HYPERPARAMETERS THAT PERFORM BEST ON AVERAGE
            if args.new_data:
                predicates, collate, train_dataset, validation_dataset, train_indices_selected_states, validation_indices_selected_states = load_dataset_new(
                    args)
                ...
                train_graphs = states_to_graphs_new(train_dataset.get_states(), predicate_dict, predicate_ids,
                                                    max_arity)
        ...
        loss_validation_all_best_val_losses = []
        loss_validation_all_best_val_coverages = []
        loss_validation_all_best_avg_plan_lengths = []

        coverage_validation_all_best_val_losses = []
        coverage_validation_all_best_val_coverages = []
        coverage_validation_all_best_avg_plan_lengths = []
        for version_dir in train_logdir.glob('version_*'):
            checkpoint_dir = version_dir / 'checkpoints'

            # TODO: FIND BEST POLICIES OF THIS RANDOM SEED
            ...
            loss_validation_all_best_val_losses.append(loss_validation_best_val_loss)
            loss_validation_all_best_val_coverages.append(loss_validation_best_val_coverage)
            loss_validation_all_best_avg_plan_lengths.append(loss_validation_best_avg_plan_length)

            coverage_validation_all_best_val_losses.append(coverage_validation_best_val_loss)
            coverage_validation_all_best_val_coverages.append(coverage_validation_best_val_coverage)
            coverage_validation_all_best_avg_plan_lengths.append(coverage_validation_best_avg_plan_length)

        loss_validation_avg_val_loss = sum(loss_validation_all_best_val_losses) / len(loss_validation_all_best_val_losses)
        loss_validation_avg_val_coverage = sum(loss_validation_all_best_val_coverages) / len(loss_validation_all_best_val_coverages)
        loss_validation_avg_avg_plan_length = sum(loss_validation_all_best_avg_plan_lengths) / len(loss_validation_all_best_avg_plan_lengths)

        coverage_validation_avg_val_loss = sum(coverage_validation_all_best_val_losses) / len(coverage_validation_all_best_val_losses)
        coverage_validation_avg_val_coverage = sum(coverage_validation_all_best_val_coverages) / len(coverage_validation_all_best_val_coverages)
        coverage_validation_avg_avg_plan_length = sum(coverage_validation_all_best_avg_plan_lengths) / len(coverage_validation_all_best_avg_plan_lengths)

        # save results
        save_results(results, policy_type="loss_validation", avg_val_loss=loss_validation_avg_val_loss, avg_val_coverage=loss_validation_avg_val_coverage,
                     avg_val_avg_plan_length=loss_validation_avg_avg_plan_length, num_layers=args.num_layers, hidden_size=args.hidden_size, dropout=args.dropout, heads=args.heads)

        save_results(results, policy_type="coverage_validation", avg_val_loss=coverage_validation_avg_val_loss, avg_val_coverage=coverage_validation_avg_val_coverage,
                     avg_val_avg_plan_length=coverage_validation_avg_avg_plan_length, num_layers=args.num_layers, hidden_size=args.hidden_size, dropout=args.dropout, heads=args.heads)
